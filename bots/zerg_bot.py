...
class BaseZergBot(BotAI):
    ...
    def can_train_drones(self) -> None:
        ...
        """
        max_drones_amount = (
            self.townhalls.amount + self.placeholders(UnitTypeId.HATCHERY).amount) * _MAX_DRONES
        drones_amount = self.supply_workers + \
            self.already_pending(UnitTypeId.DRONE)
        return self.larva and self.can_afford(
            UnitTypeId.DRONE) and drones_amount - self.worker_en_route_to_build(UnitTypeId.HATCHERY) < max_drones_amount
    ...
    async def build_spawning_pool(self) -> None:
        ...
        Build spawning pool
        """
        if self.structures(UnitTypeId.SPAWNINGPOOL).amount + \
    ...
    def build_extractor(self) -> None:
        ...
        extractors_amount = self.gas_buildings.amount + \
            self.already_pending(UnitTypeId.EXTRACTOR)
...
class CollectAndExpandBot(BaseZergBot):
    ...
    async def on_step(self, iteration: int):
        ...
        if self.supply_left < 2:
            if self.larva and self.can_afford(
                    UnitTypeId.OVERLORD) and not self.already_pending(UnitTypeId.OVERLORD):
...
class BroodlordBot(BaseZergBot):
    ...
    async def on_step(self, iteration):
        ...
        # Train Overlord
        if self.supply_left < _MIN_SUPPLY_AMOUNT and not self.already_pending(
                UnitTypeId.OVERLORD):
